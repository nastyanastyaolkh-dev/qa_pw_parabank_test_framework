...
test('Find Transactions shows error for invalid ID', async ({ existingUser, findTransactionsPage }) => {
...
test('Find Transactions shows error for invalid date', async ({ existingUser, findTransactionsPage }) => {
  await severity(Severity.NORMAL);
  await findTransactionsPage.open();
...
test('Find Transactions shows error for invalid date range', async ({ existingUser, findTransactionsPage }) => {
  await severity(Severity.NORMAL);
  await findTransactionsPage.open();
  ...
  await findTransactionsPage.assertTransactionDateRangeErrorIsShown(INVALID_DATE_RANGE);
});

test('Find Transactions shows error for invalid amount', async ({ existingUser, findTransactionsPage }) => {
  await severity(Severity.NORMAL);
  await findTransactionsPage.open();
  await findTransactionsPage.fillByAmountField('invalid');
  await findTransactionsPage.clickFindTransactionsByAmountButton();
  await findTransactionsPage.assertTransactionAmountErrorIsShown(INVALID_AMOUNT);
});
