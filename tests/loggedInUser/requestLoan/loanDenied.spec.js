...
const negativeCases = [
  { description: 'down payment exceeds account balance', overrides: { downPayment: '1000' }, expectedError: 'You do not have sufficient funds for the given down payment.' },
  { description: 'loan amount exceeds available funds', overrides: { loanAmount: '5000', downPayment: '10' }, expectedError: 'We cannot grant a loan in that amount with your available funds.' },
];

for (const testCase of negativeCases) {
  test(`Loan denied when ${testCase.description}`, async ({ existingUser, requestLoanPage }) => {
